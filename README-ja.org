...
* scad-ts-mode とは
  ...
  + OpenSCAD GUI の起動とライフサイクル管理を支援します ~(C-c C-c)~
    + GUI インスタンスがなければ，バッファ中のファイルで開きます
    + GUI インスタンスが既にあれば，何もしません (Linux なら GUI を Raise する)
    + GUI インスタンスが別ファイルを参照している場合は，起動しなおします
  + Linux 環境において
